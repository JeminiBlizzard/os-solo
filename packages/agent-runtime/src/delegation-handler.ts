 ...
 */
export function shouldProcessDelegation(output: ContentBlock[]): boolean {
 ...
 */
async function validateChildAgent(childAgentId: number): Promise<void> {
...
export async function processDelegations(
  ...
  if (context.depth >= MAX_DELEGATION_DEPTH) {
    throw new DelegationError(
      `Delegation depth limit (${MAX_DELEGATION_DEPTH}) exceeded. Cannot delegate further.`
    );
  }
 ...
 * Inherits governance from parent agent: if parent requires approval,
 * child's output goes through approval queue before being marked as completed.
 *
...
export async function executeDelegation(
  ...
  ) => Promise<{ status: string; output: ContentBlock[]; runId?: number; error?: string; approvalQueueId?: number }>,
  depth: number
): Promise<DelegationResult> {
  // Load delegation record with parent agent info
  const [delegation] = await db
    .select({
  ...
  // Load parent agent to check governance settings
  const [parentAgent] = await db
    .select({
      requiresApproval: schema.agents.requiresApproval,
      approvalThreshold: schema.agents.approvalThreshold,
    })
    .from(schema.agents)
    .where(eq(schema.agents.id, delegation.parentAgentId))
    .limit(1);

  if (!parentAgent) {
    throw new DelegationError('Parent agent not found');
  }

  try {
    // Execute child agent with delegation context
    const childInput: RunInput & { delegationDepth?: number } = {
      triggeredBy: 'chain',
    ...
    const childResult = await executeAgentFn(
      delegation.userId,
    ...
    // Determine final status based on parent governance and child result
    let finalStatus: 'pending' | 'accepted' | 'completed' | 'failed' | 'rejected';

    if (childResult.status === 'needs_approval') {
      // Child execution needs approval - delegation stays pending until approved
      finalStatus = 'pending';
    } else if (childResult.status === 'failure') {
      finalStatus = 'failed';
    } else if (parentAgent.requiresApproval) {
      // Parent requires approval - child result needs approval before returning to parent
      // Mark as 'accepted' (execution completed) but not 'completed' (not yet approved to return)
      finalStatus = 'accepted';
    } else {
      // Auto-approved parent - child can complete directly
...
export function formatDelegationResults(results: DelegationResult[]): string {
  const lines: string[] = ['Delegation results:'];
  ...
  for (const result of results) {
    if (result.status === 'completed' && result.result) {
      ...
      lines.push(
        `- Delegation ${result.delegationId} failed: ${result.error ?? 'Unknown error'}`
      );
    }
  }
