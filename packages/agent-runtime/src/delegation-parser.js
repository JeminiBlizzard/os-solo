 ...
 */
export function hasDelegationInstructions(text) {
 ...
 */
export function extractDelegationInstructions(text) {
 ...
 */
export async function parseDelegations(text, context) {
    ...
    for (const instruction of instructions) {
        ...
        const [childAgent] = await db
            ...
            .from(schema.agents)
            .where(eq(schema.agents.name, instruction.agentName))
            .limit(1);
        if (!childAgent) {
            throw new DelegationParseError(`Agent "${instruction.agentName}" not found. Cannot create delegation.`);
        }
